...
void CSkinWindow::OnNcPaintA(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	//draw text
	if(!WindowText) WindowText = (_TCHAR*)malloc(sizeof(_TCHAR)*255);
...
void CSkinWindow::OnNcPaint(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	else DrawButton(m_MemDC, 0, state*3);
	...
	else DrawButton( m_MemDC, 2, state*3 );
	...
	else DrawButton( m_MemDC, 1, state*3);
	...
	//draw text
	if(!WindowText) WindowText = (_TCHAR*)malloc(sizeof(char)*255);
	int len = GetWindowText(m_hWnd, WindowText, 255);
	...
	SetBkMode(m_MemDC, TRANSPARENT);
	SetTextColor(m_MemDC, IsActive ? RGB(255,255,255) : RGB(216, 228, 248));
...
HRGN CSkinWindow::CreateRegionFromBitmap(HBITMAP hBitmap, COLORREF transparentColor) {
	...
	if ( hBitmap != NULL ) {
		...
		if ( ( bitsPixel == 3 ) || ( bitsPixel == 4 ) ) {
			...
			hRgn = CreateRectRgn(0, 5, bitmap.bmWidth, bitmap.bmHeight);
			// Create temporary region
			hRgn1 = CreateRectRgn(5, 0, bitmap.bmWidth-5, 5);
			// Combine regions
			...
			for ( int y=0; y<5; y++ ) {
				for ( int t=0; t<2; t++ ) {
					int offset = t*(bitmap.bmWidth-5);
					for ( int x=0; x<5; x++ ) {
						// Get pixel color
						blue = pBits[y*bitmap.bmWidthBytes + bitsPixel*(x+offset)];
						green = pBits[y*bitmap.bmWidthBytes + bitsPixel*(x+offset)+1];
						red = pBits[y*bitmap.bmWidthBytes + bitsPixel*(x+offset)+2];

						// Check transparent color
						if ( RGB(red,green,blue) != transparentColor ) {
							// Combine regions
							if ( hRgn == NULL )
								hRgn = CreateRectRgn((x+offset), y, (x+offset)+1, y+1);
							else {
								...
								// Create temporary region
								hRgn1 = CreateRectRgn((x+offset), y, (x+offset)+1, y+1);
